...
def horizontal_correct(img,img_resize):
...
def ocr(img):
    ...
    cv2.imshow("img_gray_p",img_gray)
...
def main(filePath):
    ...
    # 得到姓名
    th = img_preprocess_two(expectFaceImg_origin, size=(10, 1))
    ...
    image_correct, image_resize_correct = horizontal_correct(image_id_number, img_resize)
    number = ocr(image_correct)
    print('id is :', number)
    # 识别姓名
    name = nameOcr(nameImg_origin, namePostion)
    print('name is :', name)
    return {"id":"".join(number),"name":"".join(name)}
